  ...
  return (
    <section className="py-20 bg-primary text-white overflow-hidden relative">
      <div className="absolute inset-0 opacity-10">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 rounded-full bg-emerald-400 blur-3xl" />
      </div>
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-emerald-300 text-sm font-semibold uppercase tracking-widest mb-3">Sell Smarter</p>
          <h2 className="text-3xl md:text-4xl font-bold font-heading mb-4">Ready to Make Your Move?</h2>
          <p className="text-white/85 text-lg leading-relaxed mb-8">
            List your property on RUPASINGHE REALTY and reach thousands of serious buyers. Our verified platform ensures your listing gets the right attention.
          </p>
          <div className="grid grid-cols-3 gap-4 mb-10">
            {[["Free to List", "No upfront cost"], ["Verified Buyers", "Quality inquiries only"], ["Expert Support", "Agent assistance"]].map(([t, d]) => (
              <div key={t} className="bg-white/10 rounded-xl p-4 border border-white/15">
                <CheckCircle className="w-5 h-5 text-emerald-300 mb-2 mx-auto" />
                <p className="font-bold text-sm">{t}</p>
                <p className="text-white/70 text-xs mt-1">{d}</p>
              </div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/listings/new">
              <Button className="bg-white hover:bg-white/90 text-primary font-bold px-8 h-12 rounded-xl shadow-lg">
