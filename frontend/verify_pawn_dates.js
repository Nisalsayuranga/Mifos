...
async function verifyDates() {
  const { data: pawns } = await supabase.from('pawns').select('id, description, created_at, clients(firstName, nationalId)');
  console.log('Pawns after date update:', JSON.stringify(pawns, null, 2));
}
