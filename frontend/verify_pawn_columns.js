...
async function checkExplicit() {
  const { data, error } = await supabase.from('pawns').select('id, weight, weight_grams, weight_mg').limit(3);
  console.log('--- EXPLICIT COLUMN SELECT FROM PAWNS ---');
